...
foreach ($case in $cases) {
    ...
    if ($DryRun -or -not $codex) {
        ...
        $wrappedPrompt = @"
You are running a project Codex harness trace eval. Work read-only unless this
...
"@
        for ($attemptNumber = 1; $attemptNumber -le $caseMaxAttempts; $attemptNumber++) {
            ...
            New-Item -ItemType Directory -Force -Path $attemptDir | Out-Null
            $attempt = Invoke-CodexAttempt -Codex $codex -WorkingDirectory $root -Prompt $wrappedPrompt `
