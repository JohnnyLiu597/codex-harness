...
description: Audit the active repo, MCP servers, plugins, connectors, env surfaces, and Codex harness setup, then recommend the highest-value Codex-native skills, agents, and operator workflows.
...
This is the Codex-harness answer to setup-audit plugins. It does not modify files unless the user explicitly asks for follow-up implementation.
...
- Prefer Codex-native workflows over generic "install another plugin" advice when the local harness can reasonably own the surface.
...
- Separate three things clearly:
  - already available now
   - available but not wrapped well in the Codex harness
...
1. Repo surface
   ...
   - `.mcp.json`, `.lsp.json`, `.codex/*`, and any project-local agent config
- `AGENTS.md`, project context files, install manifests, hook configs
...
- "Stripe is available via connected app, but the harness lacks a billing-operator skill"
...
- official plugins that overlap with setup, review, docs, design, or workflow quality
- locally installed plugins in Codex or adjacent agent workspaces
- the user's currently connected app surfaces

Do not just list names. For each comparison, answer:

1. what they actually do
2. whether the Codex harness already has parity
3. whether the harness only has primitives
4. whether the Codex harness is missing the workflow entirely

### Phase 3: Turn Gaps Into Codex Harness Decisions
...
2. **Parity**
   - where the Codex harness already matches or exceeds the benchmark
...
- The user can immediately see what is connected, what is missing, and what the Codex harness should own next.
