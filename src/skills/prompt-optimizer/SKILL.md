...
description: >-
  Analyze raw prompts, identify intent and gaps, match Codex harness components
...
Analyze a draft prompt, critique it, match it to Codex harness components,
...
- User is asking about harness configuration (use `project-harness-optimizer` instead)
...
1. Check if project context files exist in the working directory, then read them for project conventions
...
### Phase 3: Codex Harness Component Matching

Map intent + scope + tech stack (from Phase 0) to specific Codex harness components.
...
- "Optimize this prompt for Codex harness"
- "Rewrite this prompt so Codex uses the right skills and tools"
- "帮我优化这个指令"
...
|-----------|------------------|
| `project-harness-optimizer` | User is asking about Codex harness setup |
